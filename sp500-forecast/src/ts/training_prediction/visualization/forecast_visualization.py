...
def plot_confidence_interval(X_test, y_test, y_pred, lower, upper, test_label="Test data", pred_label="Pred", pred_interval_label="Pred intervals", offset=500):
    ...
    Parameters
    ----------
    X_test: Test dataframe containing datetime indexes and stock prices.
    y_test: Test dataframe containing actual stock prices.
...
def plot_future_forecast(df_stock_price, future_forecast, into_future=INTO_FUTURE_2_WEEK, start=100):
    """
    ...
    Parameters
    ----------
    df_stock_price: DataFrame containing datetimes and prices.
    ...
    last_timestep = df_stock_price.index[-1]
    ...
    future_forecast = np.insert(future_forecast, 0, df_stock_price.iloc[-1])

    # Plot future price predictions of stock
    plt.figure(figsize=(20, 14))
    plot_time_series(df_stock_price.index, df_stock_price, start=start, format='-', label='Actual Stock Price')
